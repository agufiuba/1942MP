...
//    escenario = new Escenario(gc);
		...
		for (int i = 0; i < this->unCliente->getPlayers().size(); i++) {
			if (this->clientId == this->unCliente->getPlayers()[i]->name) {
				...
				escenario->configurarAvionAmigo(
						this->unCliente->getPlayers()[i]);
			}
		}
...
void Game::start() {
	if (this->sdlHandler->createWindow(this->windowTitle.c_str(),
...
void Game::loadConnectionScreen() {
	...
	int logoCenter =
			(this->windowWidth - initialScreen->getTextureWidth("logo")) / 2;
	...
	while (runningScreen) {
			...
			if (e.type == SDL_QUIT) {
				...
				//Handle backspace
					...
					if (e.key.keysym.sym == SDLK_BACKSPACE
							&& this->serverIP.length() > 0) {
						//lop off character
						this->serverIP.pop_back();
						renderText = true;
					}
				} else if (e.key.keysym.sym == SDLK_BACKSPACE
						&& this->serverPort.length() > 0) {
					//lop off character
		...
		if (firstPromptSelected) {
			initialScreen->loadRectangle("outline", promptCenter,
					IPPromptOutline, 260, 50);
			initialScreen->loadRectangle("outline2", promptCenter + 1,
					IPPromptOutline2, 258, 48);
			initialScreen->loadRectangle("outline3", promptCenter + 2,
					IPPromptOutline3, 256, 46);
		} else {
			initialScreen->loadRectangle("outline", promptCenter,
					portPromptOutline, 260, 50);
			initialScreen->loadRectangle("outline2", promptCenter + 1,
					portPromptOutline2, 258, 48);
			initialScreen->loadRectangle("outline3", promptCenter + 2,
					portPromptOutline3, 256, 46);
		}
...
void Game::loadValidationScreen() {
	...
	int buttonCenter = promptCenter + 15;
	int continueTextCenter = buttonCenter
			+ ((230 - validationScreen->getTextureWidth("continue")) / 2);
	int backTextCenter = buttonCenter
			+ ((230 - validationScreen->getTextureWidth("back")) / 2);
				...
				if (connectionFailed) {
					...
					while (!endSelectingPlane) {
						PlanesActives* planes =
								this->unCliente->getPlanesActives();
...
void Game::loadTimeoutScreen() {
	...
	int logoCenter =
			(this->windowWidth - timeoutScreen->getTextureWidth("logo")) / 2;
...
void Game::loadselectionPlane() {
	...
	int buttonTextCenter = buttonCenter
			+ ((230 - initialScreen->getTextureWidth("accept")) / 2);
	int inputClientIdPosY = 250;
	int inputPlaneIdPosY = 350;
	int clientIdPromptOutline = inputClientIdPosY, clientIdPromptOutline2 =
			inputClientIdPosY + 1, clientIdPromptOutline3 = inputClientIdPosY
			+ 2;
	int bluePlanePromptOutline = inputPlaneIdPosY, bluePlanePromptOutline2 =
			inputPlaneIdPosY + 1, bluePlanePromptOutline3 = inputPlaneIdPosY
			+ 2;
	int redPlanePromptOutline = inputPlaneIdPosY, redPlanePromptOutline2 =
			inputPlaneIdPosY + 1, redPlanePromptOutline3 = inputPlaneIdPosY + 2;
	int greenPlanePromptOutline = inputPlaneIdPosY, greenPlanePromptOutline2 =
			inputPlaneIdPosY + 1, greenPlanePromptOutline3 = inputPlaneIdPosY
			+ 2;
	int yellowPlanePromptOutline = inputPlaneIdPosY, yellowPlanePromptOutline2 =
			inputPlaneIdPosY + 1, yellowPlanePromptOutline3 = inputPlaneIdPosY
			+ 2;
	...
	// Create prompts
	initialScreen->loadRectangle("promptClientId", promptCenter,
			inputClientIdPosY, 260, 50);
	initialScreen->loadRectangle("button", promptCenter + 15, 475, 230, 50);
	...
	while (runningScreen) {
		...
		initialScreen->renderTexture("logo", logoCenter, 90);
		if (bluePlaneActive) {
			initialScreen->renderTexture("bluePlane", inputBluePlaneIdPosX,
					inputPlaneIdPosY);
		}
		if (redPlaneActive) {
			initialScreen->renderTexture("redPlane", inputRedPlaneIdPosX,
					inputPlaneIdPosY);
		}
		if (greenPlaneActive) {
			initialScreen->renderTexture("greenPlane", inputGreenPlaneIdPosX,
					inputPlaneIdPosY);
		}
		if (yellowPlaneActive) {
			initialScreen->renderTexture("yellowPlane", inputYellowPlaneIdPosX,
					inputPlaneIdPosY);
		}
		...
		if (clicked) {
			...
			if ((mouseX > buttonCenter) && (mouseX < (buttonCenter + 230))
			...
			} else if ((mouseX > inputBluePlaneIdPosX)
					&& (mouseX < (inputBluePlaneIdPosX + 60))
					&& (mouseY > inputPlaneIdPosY)
					&& (mouseY < (inputPlaneIdPosY + 60))
					&& this->bluePlaneActive) {
				bluePromptSelected = true;
				redPromptSelected = false;
				greenPromptSelected = false;
				yellowPromptSelected = false;
			} else if ((mouseX > inputRedPlaneIdPosX)
					&& (mouseX < (inputRedPlaneIdPosX + 60))
					&& (mouseY > inputPlaneIdPosY)
					&& (mouseY < (inputPlaneIdPosY + 60))
					&& this->redPlaneActive) {
				...
				greenPromptSelected = false;
				yellowPromptSelected = false;
			} else if ((mouseX > inputGreenPlaneIdPosX)
					&& (mouseX < (inputGreenPlaneIdPosX + 60))
					&& (mouseY > inputPlaneIdPosY)
					&& (mouseY < (inputPlaneIdPosY + 60))
					&& this->greenPlaneActive) {
				bluePromptSelected = false;
				redPromptSelected = false;
				greenPromptSelected = true;
				yellowPromptSelected = false;
			} else if ((mouseX > inputYellowPlaneIdPosX)
					&& (mouseX < (inputYellowPlaneIdPosX + 60))
					&& (mouseY > inputPlaneIdPosY)
					&& (mouseY < (inputPlaneIdPosY + 60))
					&& this->yellowPlaneActive) {
				bluePromptSelected = false;
				redPromptSelected = false;
				greenPromptSelected = false;
		...
		initialScreen->setRenderDrawColor(19, 144, 27, 255);
		initialScreen->loadRectangle("outline", promptCenter,
				clientIdPromptOutline, 260, 50);
		initialScreen->loadRectangle("outline2", promptCenter + 1,
				clientIdPromptOutline2, 258, 48);
		initialScreen->loadRectangle("outline3", promptCenter + 2,
				clientIdPromptOutline3, 256, 46);

		if (bluePromptSelected) {
			initialScreen->loadRectangle("planeOutline", inputBluePlaneIdPosX,
					bluePlanePromptOutline, 60, 60);
			initialScreen->loadRectangle("planeOutline2",
					inputBluePlaneIdPosX + 1, bluePlanePromptOutline2, 58, 58);
			initialScreen->loadRectangle("planeOutline3",
					inputBluePlaneIdPosX + 2, bluePlanePromptOutline3, 56, 56);
		} else if (redPromptSelected) {
			initialScreen->loadRectangle("planeOutline", inputRedPlaneIdPosX,
					redPlanePromptOutline, 60, 60);
			initialScreen->loadRectangle("planeOutline2",
					inputRedPlaneIdPosX + 1, redPlanePromptOutline2, 58, 58);
			initialScreen->loadRectangle("planeOutline3",
					inputRedPlaneIdPosX + 2, redPlanePromptOutline3, 56, 56);
		} else if (greenPromptSelected) {
			initialScreen->loadRectangle("planeOutline", inputGreenPlaneIdPosX,
					greenPlanePromptOutline, 60, 60);
			initialScreen->loadRectangle("planeOutline2",
					inputGreenPlaneIdPosX + 1, greenPlanePromptOutline2, 58,
					58);
			initialScreen->loadRectangle("planeOutline3",
					inputGreenPlaneIdPosX + 2, greenPlanePromptOutline3, 56,
					56);
		} else if (yellowPromptSelected) {
			initialScreen->loadRectangle("planeOutline", inputYellowPlaneIdPosX,
					yellowPlanePromptOutline, 60, 60);
			initialScreen->loadRectangle("planeOutline2",
					inputYellowPlaneIdPosX + 1, yellowPlanePromptOutline2, 58,
					58);
			initialScreen->loadRectangle("planeOutline3",
					inputYellowPlaneIdPosX + 2, yellowPlanePromptOutline3, 56,
					56);
		}
		...
		initialScreen->renderRectangle("outline2", true);
		initialScreen->renderRectangle("outline3", true);
		if (bluePromptSelected || redPromptSelected || greenPromptSelected
				|| yellowPromptSelected) {
		...
		// Render text textures
		if (this->clientId != "") {
			initialScreen->renderTexture("clientId", textCenter,
					inputClientIdPosY);
		}
		if (error) {
			initialScreen->renderTexture("msgError", textCenter + 100,
					inputClientIdPosY);
		}

		initialScreen->renderTexture("accept", buttonTextCenter, 475);
		initialScreen->renderTexture("clientText", textoIzquierda,
				inputClientIdPosY);
		initialScreen->renderTexture("planeText", textoIzquierda,
				inputPlaneIdPosY);

		if (error) {
			initialScreen->renderTexture("msgError", textCenter + 50,
					inputClientIdPosY);
			error = false;
		}
...
void Game::loadWaitingGame() {
	...
	waitingScreen->loadText("connecting", connectingText,
			{ 255, 255, 255, 255 });

	int logoCenter =
			(this->windowWidth - waitingScreen->getTextureWidth("logo")) / 2;
	int promptCenter = logoCenter - 20;
