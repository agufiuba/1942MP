...
void Server::initialize() {
	...
	// HARDCODEADO
	if ((rv = getaddrinfo( NULL, to_string(8000).c_str(), &hints, &servinfo))
			!= 0) {
		this->logger->error(
...
void Server::listenForConnections(int cfd,
		struct sockaddr_storage client_addr) {
	// accept connections
	while (this->listening) {
		socklen_t sinSize = sizeof client_addr;
		if ((cfd = accept(this->socketFD, (struct sockaddr*) (&client_addr),
				&sinSize)) == -1) {
...
void Server::sendConf(int cfd) {
	...
	mutex theMutex;
	theMutex.lock();
	if (this->players.size()== this->config->getMaxClients()){
	GameConf* gc = GameParser::parse("gameconf.xml");
	Transmitter* tmt = new Transmitter(cfd, this->logger);
	if (!(tmt->sendData(gc))) {
		DEBUG_WARN(
				"No se pude enviar respuesta a cliente. JOB: Server::sendConf");
		this->logger->error(
				"No se pude enviar respuesta a cliente. JOB: Server::sendConf");
	}
	delete tmt;
...
void Server::receiveClientData(int cfd, struct sockaddr_storage client_addr) {
	...
	if (this->allowConnections) {
		...
		cout << endl << notice("Se inicio una conexion con el host: ")
				<< clientIP << endl;
		this->logger->info(
		...
		while (receiving) {
			...
			if (received) {
				...
				if (dataID == "PD") {
					...
					if (received = tmt->receiveData(e)) {
						...
						theMutex.lock();
						cout << endl << "FD cliente: " << notice(to_string(cfd))
								<< endl;

						map<int, Evento*>* clientMsgFD =
								new map<int, Evento*>();
			...
			if (!(received)) {
				...
				this->logger->warn(
						"El Cliente " + string(clientIP) + " se desconecto");
...
void Server::processQueue() {
	...
	while (this->processing) {
		if (!(this->eventQueue->empty())) {
			...
			this->logger->info(
					"Msj de cliente: " + to_string(it->second->value));
...
void Server::closeClient(int cfd) {
	...
	this->logger->info(
			"Cantidad de Clientes Conectados: " + to_string(this->clientCount));
	theMutex.unlock();
}
