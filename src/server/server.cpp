...
queue<Mensaje*>* msgQueue = new queue<Mensaje*>;
mutex theMutex;
...
void recieveClientData(int cfd, struct sockaddr_storage client_addr,
	...
	theMutex.lock();
	cout << endl << notice("Se inicio una conexion con el host: ") << endl << clientIP
			<< endl;
	theMutex.unlock();
...
void serverListening(int sfd, int cfd, struct sockaddr_storage client_addr,
		socklen_t sinSize) {
	// accept connections
	while (listening) {
...
//			cout << msgQueue->front()->valor << endl;
			msgQueue->pop();
			cout << "------------------------------------" << endl;
			theMutex.unlock();
		}
